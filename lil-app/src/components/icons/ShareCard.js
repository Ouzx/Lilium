import * as React from "react";
import Svg, { Path, Defs, Pattern, Use, Image } from "react-native-svg";

function SvgSharecard(props) {
  return (
    <Svg
      width={25}
      height={25}
      viewBox="0 0 25 25"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      className=""
      {...props}
    >
      <Path fill="url(#share_card_svg__pattern0)" d="M0 0h25v25H0z" />
      <Path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M11.944 12.778c0 .12.013.236.037.349l-2.113 1.056a1.667 1.667 0 100 2.189l2.113 1.057a1.667 1.667 0 10.373-.745l-2.113-1.057a1.674 1.674 0 000-.698l2.113-1.057a1.667 1.667 0 10-.41-1.094z"
        fill="#000"
      />
      <Defs>
        <Pattern
          id="share_card_svg__pattern0"
          patternContentUnits="objectBoundingBox"
          width={1}
          height={1}
        >
          <Use xlinkHref="#share_card_svg__image0" transform="scale(.02)" />
        </Pattern>
        <Image
          id="share_card_svg__image0"
          width={50}
          height={50}
          xlinkHref="data:image/png;base64,"
        />
      </Defs>
    </Svg>
  );
}

export default SvgSharecard;
